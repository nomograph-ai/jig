...
Agent-shape testing harness. Runs claude-in-the-loop task batteries
...
rubric. `jig` spawns `claude -p` subprocesses against the fixture,
records transcripts, scores them with an LLM-as-judge, and emits a
report.

## Status

Scaffold. Schema design begins at spec task t3.
